...
async def reset_state(request: Request) -> dict:
    """Clear all pipeline state. Always requires WEBHOOK_SECRET."""
    store = _get_store()
...
def _require_secret(request: Request) -> None:
    """Reject the request unless WEBHOOK_SECRET is set and matches."""
    secret = os.environ.get("WEBHOOK_SECRET", "")
    if not secret:
        raise HTTPException(
            status_code=403,
            detail="WEBHOOK_SECRET is not configured; endpoint disabled",
        )
...
def _verify_webhook_secret(request: Request) -> None:
    ...
    secret = os.environ.get("WEBHOOK_SECRET", "")
    if not secret:
        logger.warning("WEBHOOK_SECRET is not set — webhook accepted without verification")
        return
    provided = request.headers.get("X-Webhook-Secret", "")
    if not hmac.compare_digest(provided, secret):
        raise HTTPException(status_code=401, detail="Invalid secret")
...
async def github_webhook(request: Request) -> dict:
    ...
    if secret:
        ...
        logger.warning("WEBHOOK_SECRET is not set — webhook accepted without signature verification")
